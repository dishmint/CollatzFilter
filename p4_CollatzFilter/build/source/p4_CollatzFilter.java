...
float dnsmp, dscale;
 public void setup(){
	...
	source = imagefit(fpath, dnsmp); /* Use a filepath to an image as the first argument */
	// source.filter(GRAY);
 ...
 public void collatzKernelFilter(PImage srci) {
	srci.loadPixels();
	// ITERATE OVER IMAGE PIXELS
	for (int i = 0; i < srci.pixelWidth; i++){
		for (int j = 0; j < srci.pixelHeight; j++){
			// APPLY KERNEL
			int npx = collatzConvolve(srci, i, j);
 ...
 public int collatzConvolve(PImage simg, int x, int y){
 ...
 public int collatzTransmit(PImage simg, int x, int y){
